...
{
    public class AccountManager : IAccountManager<ApplicationUser, int, ApplicationRole, int>
    {
        ...
        public async Task<List<Tuple<ApplicationUser, string[]>>> GetUsersAndRolesAsync(int page, int pageSize)
        {
            ...
            var userRoleIds = users.SelectMany(u => u.Roles.Select(r => r.Id)).ToList();
            ...
            return users.Select(u => Tuple.Create(u,
                roles.Where(r => u.Roles.Select(ur => ur.Id).Contains(r.Id)).Select(r => r.Name).ToArray()))
                .ToList();
        }
        ...
        {
            if (await _context.Exams.Where(o => o.CreatedByUser.Id == userId).AnyAsync())
                return false;
        ...
        public async Task<Tuple<bool, string[]>> CreateRoleAsync(ApplicationRole role, IEnumerable<string> claims)
        {
            ...
            {
                result = await this._roleManager.AddClaimAsync(role, new Claim(Security.ClaimTypes.Permission, Permissions.GetPermissionByValue(claim)));
        ...
        public async Task<Tuple<bool, string[]>> UpdateRoleAsync(ApplicationRole role, IEnumerable<string> claims)
        {
            ...
            if (claims != null)
            {
                var roleClaims = (await _roleManager.GetClaimsAsync(role)).Where(c => c.Type == Security.ClaimTypes.Permission);
                ...
                if (claimsToAdd.Any())
                {
                    ...
                    {
                        result = await _roleManager.AddClaimAsync(role, new Claim(Security.ClaimTypes.Permission, Permissions.GetPermissionByValue(claim)));
                        if (!result.Succeeded)
                            return Tuple.Create(false, result.Errors.Select(e => e.Description).ToArray());
                    }
                }
        ...
        {
            return !await _context.Roles.Where(r => r.Id == roleId).AnyAsync();
        }
